...
class LinkedBinaryTree:
    ...
    def leaves_list(self):
        def subtree_leaves_generator(root):
            if root is None:
                return
            if root.left is None and root.right is None:
                yield root.data
                ...
                if not root.left is None:
                    yield from subtree_leaves_generator(root.left)
                if not root.right is None:
                    yield from subtree_leaves_generator(root.right)
        result = subtree_leaves_generator(self.root)
        if result is None:
